export const vertex =
  ...
  vec4 permute(const in vec4 x) {
      ...
      vec3 fromSpherical(float radius, float phi, float theta){
        ...
        float x = sinPhiRadius * sin( 0.);
        float y = cos( phi ) * radius;
        float z = sinPhiRadius * cos( theta + radius / 6.);
      ...
      void main() {
        float frequency = smoothstep(0., 1.0, uDataArr / 7000.);
        float noise = pnoise(vec2(position.x, position.y * 2.) + uTime, vec2(1.0, 0.0));
        float phi = (1. - uv.y) * PI;
        float theta = uv.x * PI * 0.25 + PI + 1.;
        vec3 pos = vec3(position);
        float r = 1.;
        pos.x *= noise * frequency;
        vec3 transform = mix(pos, fromSpherical(r, phi , theta), vec3(0.0, 1.0, 1.0));
